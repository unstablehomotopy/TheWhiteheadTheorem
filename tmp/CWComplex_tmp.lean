...
noncomputable def inclusion {A : TopCat} (X : RelativeCWComplex A) (n : ℤ) : X.sk n ⟶ X.sk (n + 1) :=
  RelativeCWComplex.AttachCells.inclusion (X.sk n) (X.sk (n + 1)) n (X.attach_cells n)

def colimitDiagram {A : TopCat} (X : RelativeCWComplex A) : ℕ ⥤ TopCat := by
  apply Functor.ofSequence
  sorry
  sorry
