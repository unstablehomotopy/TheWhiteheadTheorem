...
\hline
$\texttt{IntCat}$ & \texttt{D(}∞\texttt{-Cat)} & Ω⃗ & P⃗ & B⃗ & E⃗ & $\texttt{IntShf}$ & \texttt{D(}∞\texttt{-Cat/C)} & ω⃗ &  p⃗ & b⃗ & e⃗\\
\hline
$\texttt{IntGrpd}$ & \texttt{D(}∞\texttt{-Grpd)} & Ω⃡ & P⃡ & B⃡ & E⃡ & $\texttt{IntAct}$ & \texttt{D(}∞\texttt{-Grpd/G)} & ω⃡ & p⃡ & b⃡ & e⃡\\
 \hline
...
\begin{center}
\scalebox{0.9}{∀(C:D(∞-Cat)),∀(D:D(∞-Cat)),∀(F:C ⭢ D),∀(G:C ⭢ D),(∀(n:Nat),(π⃗ₙ F = π⃗ₙ G)) → F = G}\\
\ \\
\scalebox{0.9}{∀(X:D(∞-Grpd)),∀(Y:D(∞-Grpd)),∀(f:X ⭢ Y),∀(g:X ⭢ Y),(∀(n:Nat),(π⃡ₙ f = π⃡ₙ g)) → f = g}\\
\ \\
\scalebox{0.9}{∀(X:D(∞-Grpd₀)),∀(Y:D(∞-Grpd₀)),∀(f:X ⭢ Y),∀(g:X ⭢ Y),(∀(n:Nat),(πₙ f = πₙ g)) → f = g}\\
\ \\
\scalebox{0.9}{$\cdots$ ⭢ π⃗₁.obj C ⭢ π⃗₁.obj D $\circlearrowright$ π⃗₀.obj ((𝟙 C) • ((ω⃗.hom (𝟙 D)).hom f) ) ⭢ (π⃗₀.obj C) \stackrel{}{⭢} (π⃗₀.obj D)}\\
\ \\
\scalebox{0.9}{$\cdots$ ⭢ π⃡₁.obj E ⭢ π⃡₁.obj B $\circlearrowright$ π⃡₀.obj ((𝟙 B) • ((ω⃡.hom (𝟙 C)).hom f) ) ⭢ (π⃡₀.obj E) \stackrel{}{⭢} (π⃡₀.obj B)}\\
\ \\
...
\texttt{and material on simplicial sets}
\end{center}
...
\begin{enumerate}
\item Categories (see Mathlib's $\texttt{Category X}$; these can be bundled into $\texttt{category}$)
\item Functors (see Mathlib's $\texttt{Functor C D}$; these can be bundled into $\texttt{functor}$)
\item NaturalTransformation (see Mathlib's $\texttt{NaturalTransform F G}$; these can be bundled into $\texttt{natural\_transform}$)
\item Equations (see Mathlib's $\texttt{NatExt}$; these are related to our $\texttt{equation}$)
\end{enumerate}
...
\begin{enumerate}
\item IntCat : Cat → Cat 
\item IntShf : (X : Cat) → (C : (IntCat X)) → Cat
...
\hline
$\texttt{Symbol}$ & $\texttt{Name}$ & \texttt{Type}& \texttt{Information L/R/C} &  $\texttt{Description}$ \\
\hline
\hline
Ω⃗ & \scalebox{0.6}{The directed path space functor} &  & \scalebox{}{Representable by Δ¹} \\
\hline
P⃗ & \scalebox{0.6}{Remembrant derived directed homotopy pullback with itself} & & \\
\hline
B⃗ & \scalebox{0.6}{Remembrant derived directed homotopy pullback with itself} & & \\
\hline
E⃗ & & & \\
\hline
ω⃗ &  & & \\
...
\hline
\hline
π⃗₀ & \scalebox{0.6}{Connected components of ...} & & \\
\hline
π⃡₀ & \scalebox{0.6}{Connected components of ...} & & \\
\hline
π₀ & \scalebox{0.6}{Connected components of ...} & & \\
\hline
ρ⃗₀ & \scalebox{0.6}{Quotient of an internal $C$-presheaf} & & \\
\hline
ρ⃡₀ & \scalebox{0.6}{Quotient of an internal $G$-action (also called internal $G$-presheaf)} & & \\
\hline
...
\hline \hline
REP for ∞-categories & The replacement functor for ∞-categories\\
\hline
HEP for ∞-categories & The directed homotopy extension property\\
\hline 
The categorical Whitehead theorem & A map $\texttt{F : D(}$∞$\texttt{-Cat).Hom E B}$ is determined by $\texttt{λ(n:Nat),}$π⃗ₙ$\texttt{F}$.\\
\hline \hline
...
\hline \hline
 ...
 \hline
 $\texttt{IntShf Γ C X}$   & The category of internal presheaves \\
 \hline
...
P⃗ : D(∞-Cat) ⭢ IntCat D(∞-Cat)  & The (remembrant derived) directed path space functor \\
 \hline 
p⃗ C : D(∞-Cat⁄C) ⭢ IntShf D(∞-Cat⁄C) & The (remembrant derived) directed homotopy pullback functor\\
 \hline \hline
...
\hline \hline
The internal category fixed point principal & \texttt{D(}∞\texttt{-Cat)} is internal categories in itself \\
\hline
The internal sheaf fixed point principal & \texttt{D(}∞\texttt{-Cat⁄C)} is internal presheaves in itself \\
 \hline \hline
...
\hline \hline
REP for ∞-groupoids & The replacement functor for ∞-groupoids\\
\hline
HEP for ∞-groupoids & The homotopy extension property\\
\hline
The Whitehead theorem & A map $\texttt{F : D(}$∞$\texttt{-Grpd).Hom E B}$ is determined by $\texttt{λ(n:Nat),}$π⃡ₙ
$\texttt{F}$. \\
\hline \hline
...
\hline \hline
The internal groupoid fixed point principal & \texttt{D(}∞\texttt{-Grpd)} is internal categories in itself \\
\hline
The internal groupoid action fixed point principal & \texttt{D(}∞\texttt{-Grpd⁄C)} is internal groupoid actions in itself \\
\hline \hline
...
\hline \hline
The internal group fixed point principal & \texttt{D(}∞\texttt{-Grpd₀)} and internal groups in based spaces \\
\hline
The internal group action fixed point principal & \texttt{D(}∞\texttt{-Grpd}₀\texttt{⁄G}₀\texttt{)} and internal ΩG₀-actions in \texttt{D(}∞\texttt{-Grpd₀⁄G₀)} \\
 \hline 
\end{longtable}
}

\iffalse
\multicolumn{2}{||c||}{\texttt{Chapter 2: Pullback Systems}} \\
\hline \hline
...
The {\bf main goal} of this text is {\bf to prove the Whitehead theorem concerning based connected CW-complexes for the case of Mathlib's homotopy category of simplicial sets with a lifting condition}. The book ``Galois theories" by Borceux and Janelidze deserves special mention as an inspiration for the present project. That book details how to think about Galois theory using internal groupoids, internal G-presheaves, monadicity, comonadicity, and the constructions involved in Eilenberg Moore theory.\\
...
These functors produce what you might call ``fixed points" of the ``internal X in" operation defined on categories with pullbacks (or products in the case of the third Whitehead theorem). We obtain six categorical equivalences witnessed by these constructions. To see how, we first name the six mentioned functors I - VI in the above:
...
\begin{enumerate}
\item p⃗ (𝟙 C) : D(∞-Cat⁄C) ⭢ IntShf D(∞-Cat⁄C) Ω⃗C 
\item p⃡ (𝟙 X) : D(∞-Grpd⁄X) ⭢ IntAct D(∞-Grpd⁄X) Ω⃡X
\item p (𝟙 X₀) : D(∞-Grpd₀⁄X₀) ⭢ IntAct₀ D(∞-Grpd₀⁄X₀) ΩX₀
\end{enumerate}
\end{enumerate}

Above, the functors P⃗, P⃡, P, p⃗, p⃡, and p can be formed out of Ω⃗, Ω⃡, Ω, ω⃗, ω⃡, and ω using constructions from Eilenberg-Moore theory.\\
...
\begin{enumerate}
\item B⃗ : IntCat D(∞-Cat) ⭢ D(∞-Cat)
\item B⃡ : IntGrpd D(∞-Grpd) ⭢ D(∞-Grpd)
\item B : IntGrpd D(∞-Grpd₀) ⭢ D(∞-Grpd₀) (see \href{https://mathoverflow.net/questions/128883/why-omega-x-and-bg-are-adjoint-functors}{here})
\end{enumerate}
\item The clutching functors are inverse to the above functors up to natural isomorphism:
\begin{enumerate}
\item b⃗ : IntShf D(∞-Cat⁄C) Ω⃗C ⭢ D(∞-Cat⁄C)
\item b⃡ : IntAct D(∞-Cat⁄C) Ω⃡X ⭢ D(∞-Cat⁄C)
\item b : IntAct₀ D(∞-Grpd₀⁄X₀) ΩX₀ ⭢ D(∞-Grpd₀⁄X₀) 
\end{enumerate}
\end{enumerate}
...
\item p⃗ • b⃗ ≅ 𝟙 (IntShf D(∞-Cat⁄C) P⃗C) and b⃗ • p⃗ ≅ 𝟙 D(∞-Cat⁄C)
...
We will make extensive use of Mathlib's \href{https://leanprover-community.github.io/mathlib4_docs/Mathlib/CategoryTheory/Category/Cat.html#CategoryTheory.Cat.bicategory}{bicategory of categories}, as well as Mathlib's \href{https://leanprover-community.github.io/mathlib4_docs/Mathlib/CategoryTheory/Bicategory/Basic.html}{bicategories} in general. We further use Mathlib's pullbacks and categorical products, as well as their Eilenberg-Moore theory. I'd like to extend my appreciation to Scott Morison and all the contributors who have put their efforts into creating a great category theory section in Mathlib.\\
...
\hline
\hline
\texttt{Deductive} & \texttt{Remembrant} & \texttt{Classifying}\\
\hline
\hline
Ω⃗ \scalebox{0.7}{(Directed path space)} & P⃗ \scalebox{0.7}{(Remembrant derived directed path space)} & B⃗ \scalebox{0.7}{(Classifying space for internal categories)} \\
\hline
Ω⃡ \scalebox{0.7}{(Path space)} & P⃡ \scalebox{0.7}{(Remembrant derived path space)} & B⃡ \scalebox{0.7}{(Classifying space for internal groupoids)} \\
 \hline
Ω \scalebox{0.7}{(Loop space)} & P \scalebox{0.7}{(Remembrant derived loop space)} & B \scalebox{0.7}{(Classifying space for internal groups)} \\
 \hline
 \hline
ω⃗ \scalebox{0.7}{(Directed homotopy pullback)} & p⃗ \scalebox{0.7}{(Remembrant derived directed homotopy pullback)} & b⃗ \scalebox{0.7}{(Classifying space for internal presheaves)}\\
 \hline
ω⃡ \scalebox{0.7}{(Homotopy pullback)} & p⃡ \scalebox{0.7}{(Remembrant derived homotopy pullback)} &  b⃡ \scalebox{0.7}{(Classifying space for internal groupoid actions)} \\
 \hline
ω \scalebox{0.7}{(Homotopy fiber)} & p \scalebox{0.7}{(Remembrant derived homotopy fiber)} & b \scalebox{0.7}{(Classifying space for internal group actions)} \\
 \hline
\end{tabular}
\end{center}}
\ \\

The term ``remembrant" in the above is not common terminology. It is intended to mean that these functors ``remember" all of the structure which one can put on the derived functors of the functors in the left collumn $\textit{}$. In other words, the second collumn features functors which are valued in categories of internal objects wheras the left collumn forms particular components of those structures.\\

The notation here is both an attempt to make the various analogies manifest while sticking to familiar notation where available (such as in the case of Ω and B, which match the ordinary usage of these symbols). In the above, P could be said to stand for ``path space" and p for ``pullback", while at the same time this matches a nice theme that our capitcal letters reflect various internal structures and their lower-case forms reflect the corresponding actions.\\

The mentioned ``fixed point principals", which identify categories which are internal X's in themselves, form important consequences of the three Whitehead theorems. All in all, there are twelve important theorems we will show:\\
...
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_category\_fixed\_point\_principal : Type}$.}
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_sheaf\_fixed\_point\_principal : Type}$.}
...
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_groupoid\_fixed\_point\_principal : Type}$.}
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_groupoid\_action\_fixed\_point\_principal : Type}$.}
...
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_group\_categorical\_equivalence : Type}$.}
\item \scalebox{0.9}{Define and inhabit the $\texttt{internal\_group\_action\_categorical\_equivalence : Type}$.}
\end{enumerate}
\end{tcolorbox}
\end{center}

Note that the last two are categorical equivalences and not fixed points of "internal X in". Despite not making a perfect analogy, the three-fold nature of the table (producing three Whitehead theorems using Lean's $\texttt{Nat}$ and three Puppe sequences)
...
Here we have introduced an natural number $\texttt{n}$ using the type $\texttt{Nat}$ that comes with Lean 4. In addition to defining terms, Lean allows you to perform computations and prove theorems.\\
...
def transitivity {X : Type} {x : X} {y : X} {z : X} (p : x = y) (q : y = z) := Eq.trans ω q

\end{minted}
\end{tcolorbox}
\end{center}

This command defines a function called transitivity that proves the transitivity property of equality. It takes four type parameters: $\texttt{X}$ represents the type of the elements being compared, and $\texttt{x}$, $\texttt{y}$, and $\texttt{z}$ represent elements of type $\texttt{X}$. The function also takes two arguments ω and $\texttt{q}$. ω is a proof that $\texttt{x}$ is equal to $\texttt{y}$ ($\texttt{x = y}$), and $\texttt{q}$ is a proof that $\texttt{y}$ is equal to $\texttt{z}$ ($\texttt{y = z}$). The function body states that the result of transitivity is the proof of the composition of ω and $\texttt{q}$ using the $\texttt{Eq.trans}$ constructor, which allows you to combine two equality proofs to obtain a new one.\\
...
Even though we are only going to use strict bicategories, strict bicategories work terribly with type systems and necessitate the use of $\texttt{cast}$. Lean has only ever used bicategories for this reason. Mathlib's bicategories are also enabled by $\texttt{aesop}$, which plays a nice role in inferring simple lemmas such as ones involving identity and associativity laws. The bicategory of categories is the only bicategory we'll really need, if at all.\\
...
In the material that follows, we will be interested in forming pullbacks for four of the six internal structures that we make. We also take care to establish the definition of a fixed point of an endofunctor of Bicategory.Cat (Mathlib's category of categories). 
...
Ω⃗ is to internal categories as ω⃗ is to internal C-presheaves. As such, ω⃗ is a functor from ∞-Cat. It is also called directed homotopy pullback. These functors will later be used to produce functors P⃗ : D(∞-Cat) ⭢ IntCat D(∞-Cat) and p⃗ : D(∞-Cat⁄C) ⭢ IntShf (P⃗ C) D(∞-Cat⁄C).\\
...
\begin{tabular}{||l || l ||} 
 ...
 \hline
 $\texttt{IntShf Γ C : Cat}$ &\ Internal C-presheaves \\ 
 \hline
 The internal category principal &\ \texttt{f ×\_(B) f} forms an internal category\\
 \hline
 ...
 \hline
 p⃗ (𝟙 C) D : IntShf D(∞-Cat⁄C) (P⃗ C) &\ ω⃗ (𝟙 C) D forms a component of an internal C-presheaf \\
 \hline
\end{tabular}
\end{center}
}
...
In this chapter we define an \href{https://ncatlab.org/nlab/show/internal+category}{internal category}. Internal categories are most commonly defined on categories with enough pullbacks, but here we may also like to keep in mind that it is valuable to be able to iterate $\texttt{IntCat}$ in the way of composition of functors, ideally without having to handle any technicalities as to its being well defined. In other words, we define a the category of internal categories for all categories, with the internal category construction thereby producing an endofunctor $\texttt{IntCat}$ of Mathlib's category of categories. Hence we add to the information of the structure an assumed existence of the various pullbacks involved. This is not any sort of issue, since we only name the internal categories for which the relevant pullback does exist.\\
...
\section{\texttt{IntShf Γ C}}

The mentioned book $\texttt{Galois Theories}$ by Janelidze and Borceux features a definition of internal presheaves for an internal groupoid in chapter 7 which makes a good reference for the present discussion.\\
...
def internal_presheaf (Γ : pullback_system) (C : (IntCat Γ).Obj) : Type := sorry
...
def ShfHom (Γ : pullback_system) (C : (IntCat Γ).Obj) (F : internal_presheaf Γ C) (G : internal_presheaf Γ C) : Type := sorry
...
def ShfId₁ (Γ : pullback_system) (C : (IntCat Γ).Obj) (X : internal_presheaf Γ C) (Y : internal_presheaf Γ C) (f : ShfHom Γ C X Y) : ((ShfCmp Γ C X Y Y f (ShfIdn Γ C Y)) = f) := sorry
...
def IntShf (Γ : pullback_system) (C : (IntCat Γ).Obj) : 𝐂𝐚𝐭.Obj := {Obj := internal_presheaf Γ C, Hom := ShfHom Γ C, Idn := ShfIdn Γ C, Cmp := ShfCmp Γ C, Id₁ := ShfId₁ Γ C, Id₂ := ShfId₂ Γ C, Ass := ShfAss Γ C}
...
In this section we mention the internal category principal, which says that the pullback of any morphism with itself forms a component of an internal category in any category with pullbacks. In fact, the most general form of the theorem works for a noncommutative analogue of pullback, wheras the case of pullback gives an internal groupoid (a fact which we will show in later chapters).\\ 
...
/-
def descent_principal (Γ : pullback_system) (E : Γ.Obj.Obj) (B : Γ.Obj.Obj) (f : Γ.Obj.Hom E B) : Type :=  (!_(𝐂𝐚𝐭) (?_(𝐂𝐚𝐭) ( (M E B f))))).Cod ≃_(𝐂𝐚𝐭) (IntShf Γ) (P⃗_(Γ) E B f)
-/
...
\chapter{Chapter 6: The Categorical Fixed Point Principals}

After the construction in chapter 11, we will prove the internal category fixed point principal, which is the first categorical equivalence of the six mentioned in the introduction. We also prove in this chapter the internal C-presheaf fixed point principal, which is the second categorical equivalence of the six mentioned in the introduction. To do this, we first define B⃗ = B⃗\_(∞-ℂ𝕒𝕥) and b⃗ = b⃗\_(∞-ℂ𝕒𝕥).\\
...
The $\texttt{b⃗}$ symbol formally gives a pseudofunctor, but we can also create a model in which it is a functor. It occurs as one side of a categorical equivalence, the second of the six categorical equivalences which I have called ``fixed point principals".\\
...
\section{The Internal Category Fixed Point Principal}

The internal category fixed point principal will look something like this:
...
-- def internal_category_fixed_point_principal : Type := D(∞-ℂ𝕒𝕥) ≃ (IntCat D(∞-ℂ𝕒𝕥))
...
-- def internal_category_fixed_point_principal_proof : internal_category_fixed_point_principal := {Fst := internal_category_fixed_point_principalFst, Snd := internal_category_fixed_point_principalSnd, Id₁ := internal_category_fixed_point_principalId₁, Id₂ := internal_category_fixed_point_principalId₂}
...
The internal presheaf fixed point principal consists of a categorical equivalence between D(∞-Cat/C) and internal $C$-presheaves in D(∞-Cat/C).\\
...
def internal_presheaf_fixed_point_principal (C : D(∞-ℂ𝕒𝕥).Obj) : Type := Shf_(∞-ℂ𝕒𝕥) (P_(∞-ℂ𝕒𝕥) C C (𝟙_(D(∞-ℂ𝕒𝕥)) C)) ≃_(𝐂𝐚𝐭) (!_(𝐂𝐚𝐭) (?_(𝐂𝐚𝐭) (¡_(𝐂𝐚𝐭) (¿_(𝐂𝐚𝐭) (p_(∞-ℂ𝕒𝕥) C C (𝟙_(D(∞-ℂ𝕒𝕥)) C)))))).Cod

\end{minted}
\end{tcolorbox}
\end{center}

Next we prove the internal C-sheaf fixed point principal. This says says that \texttt{Shf\_(∞-ℂ𝕒𝕥) (P\_(∞-ℂ𝕒𝕥) C C (𝟙\_(D(∞-ℂ𝕒𝕥)) C)) ≃\_(𝐂𝐚𝐭) !? (p\_(∞-ℂ𝕒𝕥) C C (𝟙\_(D(∞-ℂ𝕒𝕥)) C))}.\\

%LEAN: The internal C-sheaf fixed point principal
\begin{center}
\begin{tcolorbox}[width=5in,colback={white},title={\begin{center}\texttt{Lean \thelcounter} \addtocounter{lcounter}{1}  \end{center}},colbacktitle=Yellow,coltitle=black]
\begin{minted}[breaklines, escapeinside=||]{lean}
...
def internal_presheaf_fixed_point_principal_proof (C : D(∞-ℂ𝕒𝕥).Obj) : internal_presheaf_fixed_point_principal C := {Fst := internal_presheaf_fixed_point_principalFst, Snd := internal_presheaf_fixed_point_principalSnd, Id₁ := internal_presheaf_fixed_point_principalId₁, Id₂ := internal_presheaf_fixed_point_principalId₂}
...
The main idea here is to treat this by induction, extending a homotopy for each n to a homotopy for n+1. This gives a picture that is a bit like ``filling up a jar``: a homotopy h : I × bΔ² of f, g : bΔ² ⭢ Y, along with the value of g on Δ², produces a ``jar" shape in Y, which can be ``filled up" to produce a homotopy h : I × Δ² ⭢ Y. This is easier for simplicial-based approaches than for point-set topological approaches, the latter of which needs extra steps that deform a map into a cellular map.\\
...
Ω⃡ is to internal groupoids as ω⃡ is to internal G-presheaves. ω⃡ is also called homotopy pullback, but this by no means standard notation for homotopy pullback at all. These functors will later be used to produce functors P⃗ : D(∞-Cat) ⭢ IntCat D(∞-Cat) and p⃗ : D(∞-Cat⁄C) ⭢ IntShf D(∞-Cat⁄C) (P⃗ C).\\
...
After the last section is complete, we will be in a place to prove the internal groupoid and internal groupoid presheaf fixed point principals, which are the third and fourth of the six categorical equivalences mentioned in the introduction.\\
...
def internal_groupoid_fixed_point_principal (Γ : pulback_system) : Type := D(Γ) ≃_(𝐂𝐚𝐭) Grpd_(Γ)
...
-- def internal_groupoid_fixed_point_principal_proof : internal_category_fixed_point_principal ∞-ℂ𝕒𝕥 := sorry
...
-- def internal_groupoid_presheaf_fixed_point_principal (Γ : pullback_system) (C : D(Γ).Obj) : Type := Shf_(Γ) (P⃡ (𝟙_(D(Γ)) C)) ≅ Der_(Γ) C
...
def internal_groupoid_action_fixed_point_principal (Γ : pullback_system) (C : D(Γ).Obj) : Type := Shf_(Γ) (P_(Γ) C C (𝟙_(D(Γ)) C)) ≃_(𝐂𝐚𝐭) Der_(Γ) C
...
-- def internal_groupoid_action_fixed_point_principal_proof (C : D(∞-ℂ𝕒𝕥).Obj) : internal_presheaf_fixed_point_principal ∞-ℂ𝕒𝕥 C  := sorry
