...
\hline
$\texttt{IntCat}$ & \texttt{D(}∞\texttt{-Cat)} & Σ⃗ & Ω⃗ & P⃗ & B⃗ & E⃗ & $\texttt{InfPreShf}$ & \texttt{D(}∞\texttt{-Cat/C)} & σ⃗ & ω⃗ &  p⃗ & b⃗ & e⃗\\
\hline
$\texttt{IntGrpd}$ & \texttt{D(}∞\texttt{-Grpd)} & Σ⃡ & Ω⃡ & P⃡ & B⃡ & E⃡ & $\texttt{IntAct}$ & \texttt{D(}∞\texttt{-Grpd/G)} & σ⃡ & ω⃡ & p⃡ & b⃡ & e⃡\\
 \hline
...
\begin{center}
\scalebox{0.85}{∀(C:D⃗(∞-Cat)),∀(D:D⃗(∞-Cat)),∀(F:C ⭢ D),∀(G:C ⭢ D),(∀(n:Nat),(π⃗ₙ F = π⃗ₙ G)) → F = G}\\
\ \\
\scalebox{0.85}{∀(X:D⃡(∞-Grpd)),∀(Y:D⃡(∞-Grpd)),∀(f:X ⭢ Y),∀(g:X ⭢ Y),(∀(n:Nat),(π⃡ₙ f = π⃡ₙ g)) → f = g}\\
\ \\
...
\texttt{three variations of the Puppe sequence}\\
\texttt{of homotopy groups in Lean 4, with}\\
\texttt{extensive use of Mathlib 4}
\end{center}
...
{\bf The main goal of this repository is to prove the Whitehead theorem}. It is important that initial pull requests stemming from our work remain basic and accessible; we hope to make progress which is gradual and incremental.\\
...
\item e⃗ : InfPreShf ∞-Cat⁄C P⃗.obj C ⭢ ∞-Cat⁄C P⃗.obj C
...
\begin{center}
\iffalse
\includegraphics[width=7.5cm,height=5cm]{about2.jpg}
\fi
\end{center}
\end{tcolorbox}
\end{center}
