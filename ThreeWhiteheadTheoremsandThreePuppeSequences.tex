...
\hline
$\texttt{IntCat}$ & \texttt{D⃗(}∞\texttt{-Cat)} & Σ⃗ & Ω⃗ & P⃗ & B⃗ & E⃗ & $\texttt{IntPrShf}$ & \texttt{D⃗(}∞\texttt{-Cat/C)} & σ⃗ & ω⃗ &  p⃗ & b⃗ & e⃗\\
\hline
$\texttt{IntGrpd}$ & \texttt{D⃡(}∞\texttt{-Grpd)} & Σ⃡ & Ω⃡ & P⃡ & B⃡ & E⃡ & $\texttt{IntAct}$ & \texttt{D⃡(}∞\texttt{-Grpd/G)} & σ⃡ & ω⃡ & p⃡ & b⃡ & e⃡\\
 \hline
$\texttt{IntGrp}$ & \texttt{D(}∞\texttt{-Grpd₀)} & Σ & Ω & P & B & E & $\texttt{IntAct₀}$ & \texttt{D(}∞\texttt{-Grpd₀/G₀)} & σ & ω & p & b & e\\
 \hline
\end{tabular}}
\end{center}}
\ \\
...
??? & ???\\
\hline
\multicolumn{2}{||c||}{\texttt{Chapter 18: }Colimits and Homotopy Colimits} \\
\hline
??? & ???\\
\hline
...
https://math.stackexchange.com/questions/4825283/b-of-a-pseudo-group
\fi
...
\item The theorem that P • B ≅ 𝟙 (essential${\_}$image P) and B • P ≅ 𝟙 D(∞-Grpd₀)
...
/-
-- def internal_groupoid_presheaf_delooping_principal (Γ : pullback_system) (C : ∞-...) : Type := Shf_(Γ) (P⃡ (𝟙_(D(∞-Cat)) C)) ≅ Der_(∞-Cat) C
-/
...
The last axiom above is a bit tricky, but it is related to straightening and unstraightening.\\
...
The pullback system will be able to encapsulate information associated to ∞-Cat, D(∞-Cat), and D(∞-Cat⁄C), namely the derived directed homotopy pullback functor and its adjoint D(ω𛲔.hom f), derived postcomposition D(ωॱ.hom f).\\
...
The $\textit{straightening}$ of \texttt{χ.obj F : D ⭢ ∞\_(∞-Cat)} an ∞-functor F : C ⭢ D is an object in ∞-Cat ⁄ C.
