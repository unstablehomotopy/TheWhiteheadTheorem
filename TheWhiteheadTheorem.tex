...
\begin{proof}
For each $\texttt{n : Nat}$, let $\texttt{Xₙ}$ be the nth space in the complex $\textt{X}$, and let $\texttt{αₙ : ∞-Grpd₋₁.Hom (Aₙ × Sⁿ) Xₙ}$ be the attaching map. Let $\texttt{fₙ}$ be the precomposition of $\texttt{f}$ with the map $\texttt{∞-Grpd₋₁.Hom Xₙ X}$. We construct a homotopy $\texttt{f ≃ *}$ inductively. 
\end{proof}
