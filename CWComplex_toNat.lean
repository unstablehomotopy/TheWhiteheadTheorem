...
lemma jarProj_compatible (n : ℤ) {Y : Type} [TopologicalSpace Y]
  ...
  fun ⟨⟨x, hx⟩, ⟨y, hy0, hy1⟩⟩ hp0 hp1 ↦ by
    ...
    conv in jarRimProj n _ => equals @inc₀ (𝕊 n) q =>
      unfold jarRimProj jarRimProjFst jarRimProjFstToFun jarRimProjSnd jarRimProjSndToFun inc₀
    ...
    change (bundledSphereInclusion n ≫ f).toFun q = (inc₀ ≫ H).toFun q
    rw [hf]

lemma jarProj_compatible' (n : ℤ) {Y : TopCat}
...
noncomputable def jarHomotopyExtension (n : ℤ) {Y : TopCat}
    (f : TopCat.of (𝔻 n + 1) ⟶ Y) (H: TopCat.of ((𝕊 n) × I) ⟶ Y)
    (hf: bundledSphereInclusion n ≫ f = inc₀ ≫ H) : TopCat.of (Jar n) ⟶ Y :=
...
lemma jarHomotopyExtension_bottom_commutes (n : ℤ) {Y : TopCat}
    (f : TopCat.of (𝔻 n + 1) ⟶ Y) (H: TopCat.of ((𝕊 n) × I) ⟶ Y)
    (hf: bundledSphereInclusion n ≫ f = inc₀ ≫ H) :
...
lemma jarHomotopyExtension_wall_commutes (n : ℤ) {Y : TopCat}
    (f : TopCat.of (𝔻 n + 1) ⟶ Y) (H: TopCat.of ((𝕊 n) × I) ⟶ Y)
    (hf: bundledSphereInclusion n ≫ f = inc₀ ≫ H) :
    H = ContinuousMap.prodMap (bundledSphereInclusion n) (𝟙 (TopCat.of I)) ≫ jarHomotopyExtension n f H hf := by
  ext p
  change H p = (jarHomotopyExtension n f H hf) (prodMap _ (𝟙 (TopCat.of I)) p)
  obtain ⟨x, y⟩ := p
  rw [ContinuousMap.prod_eval]
  sorry
