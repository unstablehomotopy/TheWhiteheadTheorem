...
\ \\
\scalebox{0.85}{∀(X:D(∞-Grpd)),∀(Y:D(∞-Grpd)),∀(f:D(∞-Grpd).Hom X Y),∀(g:D(∞-Grpd).Hom X Y),(∀(n:Nat),(π⃡ₙ f = π⃡ₙ g)) → f = g}\\
\ \\
...
\item We model ∞-Grpd₀ : Cat firstly as the BASED objects using Mathlib 4's topological spaces, and secondly as the category of BASED Kan complexes.
\end{enumerate}
...
Algebra for an A-infinity operad / Algebra for an A-infinity operoid
Algebra for an E-infinity operad / Algebra for an E-infinity operoid

Grouplike algebra for an A-infinity operad / Groupal algebra for an A-infinity operoid
Grouplike algebra for an E-infinity operad / Groupal algebra for an E-infinity operoid
...
\section{REP for BASED ∞-groupoids}
...
The Whitehead theorem is about the ways that spheres can get stuck in spaces (higher homotopy groups), and the last section established how these higher homotopy groups relate to maps in the homotopy category of BASED CW-complexes. There is the nuance that the base .\\
