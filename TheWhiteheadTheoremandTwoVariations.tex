...
\begin{center}
\ \\
\scalebox{3}{\texttt{𝕋𝕙𝕖 𝕎𝕙𝕚𝕥𝕖𝕙𝕖𝕒𝕕 𝕋𝕙𝕖𝕠𝕣𝕖𝕞}}\\
\ \\
...
\hline
$\texttt{IntCat}$ & \texttt{D(}∞\texttt{-Cat)} & R⃗ & Ω⃗ & P⃗ & $\texttt{InfPreShf}$ & \texttt{D(}∞\texttt{-Cat/C)} & r⃗ & ω⃗ & p⃗ \\
\hline
$\texttt{IntGrpd}$ & \texttt{D(}∞\texttt{-Grpd)} & R⃡ & Ω⃡ & P⃡  & $\texttt{IntAct}$ & \texttt{D(}∞\texttt{-Grpd/G)} & r⃡ & ω⃡ & p⃡\\
 \hline
$\texttt{IntGrp}$ & \texttt{D(}∞\texttt{-Grpd₀)} & R & Ω & P  & $\texttt{IntAct₀}$ & \texttt{D(}∞\texttt{-Grpd₀/G₀)} & r & ω & p \\
 \hline
\end{tabular}}
\end{center}}
\ \\
...
\iffalse
We keep a separate, fourth whitehead theorem for Hilbert space, like so:
\fi
