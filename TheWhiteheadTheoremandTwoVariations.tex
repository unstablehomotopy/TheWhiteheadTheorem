...
We also maintain a fourth Whitehead theorem for Hilbert space:

\begin{enumerate}
\item (The Whitehead theorem for Hilbert space) ∀(v:l₂),∀(w:l₂),∀(n:Nat),∀(a:πᵢ(v)),∀(b:πⱼ(v)), where πᵢ : l₂ ⭢ ℂ is notation for ⟨eᵢ,-⟩
\end{enumerate}

\iffalse
There is no base in the variations, and we instead consider an object with two bases when forming Ω⃗ and :
({f : γ ⭢ A // T f t = T f s for each s, t}, {(f,g,x,y) : [γ,C] × [γ,C] × C × C // T f = (x,y), and T g = (x,y)})
\fi
...
\iffalse
3 goals (and two extras for connected...)
6 internal structures (and two extras for connected...)
6 models (and two extras for connected...)
6 derived models
6 Ω's
\fi
...
\href{https://ncatlab.org/nlab/show/globular+set}{Globular sets} are functors from the globe category 𝔾 into the category of sets, and maps of globular sets are natural transformations between them.\\
...
In this section we prove the homotopy extension property for based connected ∞-groupoids, which is here the object component of ∞-Grpd₀.\\

\begin{center}
\includegraphics[width=3.5cm,height=7cm]{proj.png}
\end{center}
...
\begin{theorem}
Fix n : ℕ and let i : Sⁿ ⭢ Dⁿ⁺¹ be the inclusion. The pushout of the following diagram is isomorphic to Sⁿ⁺¹:
\begin{center}
...
{\bf Jar filling} Next we turn to defining `jar shapes' $\texttt{Jⁿ}$, which include into $\texttt{Dⁿ × I}$ $\texttt{iₙ : Jₙ ⭢ Dⁿ × I}$, after which we `fill' them by way of extension . The jar shape is 

The first way to fill the jar
...
\begin{definition}
Let X₋₁ be a connected CW-complex and let n : Nat be a natural number. The transport function $\texttt{trans\ n\ X₋₁ : (f : [I,X₋₁])  → πₙ (f 0) ⭢ πₙ (f 1)}$ is given by...
\end{definition}
...
\part{BASED CONNECTED ∞-MONOIDS}


\chapter{Chapter 5: ∞\texttt{-Cat₀}}

Here we define the categories D(∞-Grpd₀) of connected based ∞-groupoids and D(∞-Grpd₀/G₀) using CW-complexes.\\
...
\chapter{Chapter 6: The Whitehead Theorem for ???}
...
\section{The Whitehead theorem for ???}
...
\chapter{Chapter 8: Internal and Operadic Groups}


\iffalse
An operadic group is an algebra for the little lines operad with 6 entries

An operadic internal group in C is an algebra in C for the little lines internal operad with ??? entries
\fi



\section{\texttt{InternalMonoid}}
...
\section{\texttt{InternalMonoidAction G}}

Here we define internal group actions. These will be important when we talk about G-principal bundles (themselves defined as internal group actions in the derived category of an overcategory). 


\section{\texttt{OperadicMonoid}}




\section{\texttt{OperadicMonoidActions}}



\section{The ??? and ??? Functors}
...
\section{\texttt{InternalGroupoid Γ}}
...
\section{Operadic Groupoid}


\section{Operadic Groupoid Action}
...
\section{\texttt{InternalPresheaf Γ C}}
